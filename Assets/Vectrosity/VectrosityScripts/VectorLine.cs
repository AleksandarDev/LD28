...
public class VectorLine {
	...
	public int drawStart {
		...
		set {
			...
			m_drawStart = Mathf.Clamp(value, 0, m_is2D? points2.Length-1 : points3.Length-1);
		}
	}
	int m_drawEnd = 0;
	public int drawEnd {
		...
		set {
			...
			m_drawEnd = Mathf.Clamp(value, 0, m_is2D? points2.Length-1 : points3.Length-1);
		}
	}
	...
	public void SetColor (Color color, int startIndex, int endIndex) {
		int max = m_continuous? m_pointsLength-1 : m_pointsLength/2;
		startIndex = Mathf.Clamp (startIndex, 0, max-1);
		endIndex = Mathf.Clamp (endIndex + 1, 1, max);
		endIndex *= 4;
	...
	private bool CheckLine () {
		...
		if (m_useMeshQuads) {
			if (m_joins != Joins.Fill) {
				...
				if (m_drawEnd > 0 && m_drawEnd < (m_is2D? points2.Length-1 : points3.Length-1)) {
